...
function primeiroNome(nomeCompleto) {
  if (typeof nomeCompleto !== 'string') {
    return '';  // Garantir que seja uma string
  }
  const nomeSemEspacos = nomeCompleto.trim(); 
  const espacoEmBranco = nomeSemEspacos.indexOf(" ");
  if (espacoEmBranco === -1) return nomeSemEspacos; 
  return nomeSemEspacos.slice(0, espacoEmBranco); 
}
...
function verificarDisponibilidadeEstoque(tipoProduto, quantidade) {
  ...
  if (estoqueDisponivel === undefined || estoqueDisponivel < quantidade) {
    return false; 
  }
  return true;
}
